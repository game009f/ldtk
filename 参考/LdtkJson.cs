...
namespace ldtk
{
    ...
    ///本节包含所有级别数据。它有两种不同的形式，具体取决于
    ///在项目当前设置中：-如果“*独立级别文件*”被**禁用**（默认值）：
    ///完整级别的数据*嵌入*在主项目JSON文件中，-If“*单独级别
    ///文件*“已启用**：级别数据存储在*单独的*独立的`.ldtkl`文件中（一个
    ///每个级别）。在这种情况下，主项目JSON文件仍将包含大多数级别的数据，
    ///除了较重的部分，比如'layerInstances'数组（将为空）。这个
    ///'externalRelPath'字符串指向'ldtkl'文件。ldtkl文件只是一个JSON文件
    ///包含下面描述的内容。
    /// </summary>
    public partial class Level
    {
        ...
        ///包含所有图层实例的数组**重要提示**：如果项目选项“*保存”
        ///如果启用了levels separated*，则此字段将为空。<br/>此数组已**排序
        ///按显示顺序**：第一层是最上面的，最后一层在后面。
        /// </summary>
